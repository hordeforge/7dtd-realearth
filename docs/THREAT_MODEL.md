...
Last reviewed: 2026-08-26 · against commit `bb410a8`.
...
| 3 | T3 | Operator binds `realearth serve` past loopback: packs served unauthenticated | B4 viewer server | `tools/realearth/cli.py:675-711`; server `tools/realearth/viewer_server.py:140-155` | Defaults to 127.0.0.1 + loud warning on any other bind (`cli.py:694-699`); static-only with containment and CSP (`viewer_server.py:60-71,97-107`) |
| 4 | T4 | Build chain compromise or post-patch DLL drift silently runs expanded code nobody verified | B6 build → runtime | `tools/engine_patcher/Program.cs:241-289`; `scripts/apply_engine_expand.sh` | Backup + marker + dry-run + restore; expand records sha256 and `make engine-verify` detects drift (`Program.cs:298-360`; `Makefile:225-227`), but verification is opt-in |
...
| E6 | Pipeline CLI (`demo`,`region`,`bake-world`,`export-viewer`,`inspect-tile`,`serve`) | GeoJSON files, GeoTIFFs, CLI args, remote API responses; echoed pack strings sanitized; world names joined onto paths are component-checked | `tools/realearth/cli.py:19,31-56,215` |
| E7 | Third-party fetches in pipeline | open-meteo elevation API, AWS terrarium PNG tiles | `tools/realearth/elevation.py:74,142` |
| E8 | Static viewer server + browser app | Same-origin JSON/PNG; user-supplied local JSON file; optional same-origin `data/player.json` position feed (coerced/range-checked, absence tolerated). Server: translated-path containment, directory listings off, security headers, control chars stripped from logs | `cli.py:675-711`; `viewer_server.py:45-53,60-77,94-107`; `viewer/src/pack.ts:155-196`; `viewer/src/app.ts:80,288,499` |
| E9 | Install/expand scripts + IL patcher | Writes into Steam dirs; patches game DLL; lint gates fetch SHA-pinned tarballs; CI pins Actions by commit SHA and gates release tags against shipped version | `scripts/install_proton.sh:24-28,75,209`; `engine_patcher/Program.cs:193-232`; `scripts/lint-webmod.sh:26`, `lint-viewer.sh:27`; `.github/workflows/release.yml:16-42` |
...
- **B3 Pack author → operator → game/pipeline/viewer.** `.rte`, `earth.manifest.json`, `settlements.json`, GeoTIFFs. No signatures; manifest notes are free text. Downstream echo surfaces sanitize (CLI `_display_text` `cli.py:31-38`; viewer server logs `viewer_server.py:45-53`); path-joining of pack names rejects traversal components (`cli.py:41-56`).
- **B4 Browser ↔ viewer/webmod.** Viewer origin renders pack strings through text-only DOM APIs; served responses carry CSP/nosniff/frame-deny (`viewer_server.py:60-71`); translated paths must stay inside the served tree (`viewer_server.py:97-107`). WebMod executes within the game's authenticated dashboard session (game owns that boundary); its pack/file fields pass the same path guard as the viewer (`webmod/src/pack.ts:17-32,70,108`).
- **B5 Operator ↔ install tooling.** Scripts remove guarded destinations inside Steam dirs (`install_proton.sh:75,209`); gated by explicit `GAME_DIR` existence checks (`install_proton.sh:24-28`; Makefile setup). Operator trust assumed; not sandboxed.
...
| Viewer server: path containment, listings off, CSP/nosniff/frame-deny/referrer-policy, log sanitization | T3, T7 support | `viewer_server.py:45-53,60-77,94-107` |
...
| CLI echo sanitization + safe name components for pack-controlled strings | terminal/log injection, path escape | `cli.py:31-56` |
| Engine patch: backup/marker/dry-run/restore + sha256 recorded at expand + `engine-verify` drift check | T4 | `Program.cs:193-232,298-360`; `Makefile:225-227` |
| `GAME_DIR` existence checks before install/destructive rm | wrong-target destruction | `Makefile` setup; `install_proton.sh:24-28` |
| Dependency pinning (`uv.lock`), SHA-pinned CI Actions, release-tag/version gate | B2/B6 supply chain | `tools/uv.lock`; `.github/workflows/release.yml:16-42` |
...
3. `realearth serve --bind` past loopback serves everything under the viewer root unauthenticated by design; the warning (`cli.py:694-699`) is the only control (T3).
