...
Last reviewed: 2026-08-26 · against commit `a9212e4`.
...
| 7 | T6 | Operator clones template and re-enables telnet on an exposed host | B7 deployment | `scripts/serverconfig_height_test.xml:44-46` (`TelnetEnabled=false`, `TelnetPassword` empty) | Template ships telnet off with empty password; re-enable comment demands a strong one |
...
- **Not held:** user PII, payment data, signing keys, stored credentials (repo scan finds no secret-shaped artifacts; the test telnet config ships disabled with an empty password, `scripts/serverconfig_height_test.xml:44-46`).
...
| E3 | Mod config JSON at init | Operator values incl. `TileCdnBaseUrl`, debug flags; validated at startup with logged warnings | `ModApi.cs:21`; `RealEarthConfig.cs:171-215` |
| E4 | Game console commands (`reheight`,`rereveal`,`recities`,`reinject`,`resession`) | Admin-only via console/telnet (game-owned auth); map reveal additionally config-gated | `Source/RealEarth/ConsoleCmdRe*.cs`; `MapReveal.cs:38` |
...
| E7 | Third-party fetches in pipeline | open-meteo elevation API and AWS terrarium PNG tiles (retry-bounded GETs); fetched tiles persist to a disk cache (`RE_TERRARIUM_CACHE`, default `data/cache/terrarium`) that later builds trust without revalidation | `tools/realearth/elevation.py:23-51,79,151,156-208`; `Makefile:59` |
| E8 | Static viewer server + browser app | Same-origin JSON/PNG; user-supplied local JSON file; optional same-origin `data/player.json` position feed (coerced/range-checked, absence tolerated); **remote script origin**: importmap loads three.js from cdn.jsdelivr.net. Server: translated-path containment, directory listings off, security headers, control chars stripped from logs | `cli.py:818-862`; `viewer_server.py:48-58,62-77,85,106-118`; `viewer/src/app.ts:80,288,498-500`; `viewer/index.html:12-17` |
| E9 | Install/expand scripts + IL patcher (+ offline `network_protocol_inspector` dumper) | Writes into Steam dirs; patches game DLL; restore extracts operator-supplied tar archives into the repo. Lint/build tool pins centralized in one sourced env file (anti-slop tarball by commit SHA, packages by version); CI pins Actions by commit SHA and gates release tags against shipped version | `scripts/install_proton.sh:24-31,75,174`; `engine_patcher/Program.cs:193-232`; `network_protocol_inspector/Program.cs:3`; `scripts/toolchain-versions.env:20-39`; `.github/workflows/release.yml:22-45` |
...
- **B2 Third-party APIs → pipeline workstation → durable cache.** open-meteo/S3 responses decoded offline (`elevation.py`). No validation beyond PNG/library decode, and fetched Terrarium tiles now persist to `data/cache/terrarium` (`elevation.py:162-183`) where later builds serve them back without any re-check (`elevation.py:259-264`): a poisoned or silently corrupted cache becomes world truth on every future rebuild (extends T2's persistence half to the pipeline). Deps pinned by `tools/uv.lock`.
- **B3 Pack author → operator → game/pipeline/viewer.** `.rte`, `earth.manifest.json`, `settlements.json`, GeoTIFFs. No signatures; manifest notes are free text. Downstream echo surfaces sanitize (CLI `_display_text` `cli.py:37-44`; viewer server logs `viewer_server.py:48-58`); path-joining of pack names rejects traversal components (`cli.py:47-66`); C# ingestion normalizes names to NFC and saturates out-of-range populations (`CityMapLabels.cs:47-48,731-744`).
- **B4 Browser ↔ viewer/webmod (+ CDN script origin).** Viewer origin renders pack strings through text-only DOM APIs; served responses carry CSP/nosniff/frame-deny (`viewer_server.py:62-77`) with listings blocked by an override (`viewer_server.py:85`); translated paths must stay inside the served tree (`viewer_server.py:106-118`). The CSP also whitelists `https://cdn.jsdelivr.net` for scripts (`viewer_server.py:73`) because the importmap pulls three.js from there at runtime (`viewer/index.html:16-17`): a third-party script origin inside the viewer trust boundary (T8). WebMod executes within the game's authenticated dashboard session (game owns that boundary); its pack/file fields pass the same path guard as the viewer (`webmod/src/pack.ts:17-32,70,108,229`).
- **B5 Operator ↔ install tooling / backup tooling.** Scripts remove guarded destinations inside Steam dirs (`install_proton.sh:75,174`); gated by explicit `GAME_DIR` existence checks (`install_proton.sh:24-31`; Makefile setup). Config and serverconfig writing runs as `realearth.mod_config` / `realearth.server_config` with values passed as argv, so an env-supplied world name or userdata path is data, never Python or XML source; the previous heredocs interpolated them into a script body. The height-test harness moves old saves into a 7-day trash window instead of deleting them (`run_dedicated_height_test.sh:160-173`). Artifact restore verifies sha256/gzip and refuses to clobber existing dirs unless `RE_FORCE_RESTORE=1`, but otherwise trusts the archive it is pointed at (`backup_artifacts.sh:88-122`). Operator trust assumed; not sandboxed.
- **B6 Build → runtime.** Patcher rewrites the managed game DLL; output gains full game-process authority. Marker + stock backup + late-backup convergence exist (`Program.cs:193-232`); the written bytes' sha256 is recorded in the marker (`Program.cs:298-310`) and `--verify` compares current bytes against it (`Program.cs:326-360`, Makefile `engine-verify:248-251`). Release workflow rejects a tag that disagrees with `ModInfo.xml` (`.github/workflows/release.yml:22-45`) and pins Actions by commit SHA. Lint/build tooling is pinned in one sourced file: anti-slop tarball by commit SHA (`toolchain-versions.env:31`), all npm packages by version (`toolchain-versions.env:20-39`), consumed by every build/lint script.
- **B7 Secrets → code.** No credentials stored or rotated here. Telnet ships disabled with an empty password (`scripts/serverconfig_height_test.xml:44-46`); CDN URL is configuration, not a secret.
...
- **B2:** Tampering: hostile upstream elevation/population silently becomes world truth; no cross-source check. New persistence half: poisoned Terrarium tiles land in `data/cache/terrarium` and outlive the hostile run (`elevation.py:170-183`). DoS: large PNG decode on a workstation; low impact offline.
...
- **B5:** Data loss: wrong env var redirects destructive paths; mitigated by existence checks and the saves trash window, not dry-run defaults. Restore pointed at a hostile archive writes wherever its members name (operator-supplied input assumed benign).
...
| Engine patch: backup/marker/dry-run/restore + sha256 recorded at expand + `engine-verify` drift check | T4 | `Program.cs:193-232,298-360`; `Makefile:248-251` |
| `GAME_DIR` existence checks before install/destructive rm | wrong-target destruction | `Makefile` setup; `install_proton.sh:24-31` |
| Artifact backup/restore: gzip + sha256 verification before declare/restore; restore refuses existing dirs unless forced (moves aside); harness saves go to a dated trash window | B5 durability/destructive-path threats | `backup_artifacts.sh:61-68,92-119`; `run_dedicated_height_test.sh:160-173` |
...
| Dependency pinning (`uv.lock`, committed NuGet lock files), centralized lint/build pins (anti-slop tarball by commit SHA; npm packages by version), SHA-pinned CI Actions, release-tag/version gate | B2/B6 supply chain | `tools/uv.lock`; `scripts/toolchain-versions.env:20-39`; `.github/workflows/release.yml:22-45` |

### Gaps (ranked; fixes go to sec-review)

1. No authenticity check for tiles/packs anywhere in the chain: no signature, no hash pinning; runtime CDN fetch, installed packs, *and* the pipeline's Terrarium disk cache (`elevation.py:162-183`) all trust bytes on transport alone, so transport hardening does not help against the CDN itself or a malicious pack publisher (T1, T2).
...
3. `realearth serve --bind` past loopback serves everything under the viewer root unauthenticated by design; the warning (`cli.py:839-845`) is the only control (T3).
...
Documented-but-not-implemented check: no security-relevant claim in `README.md`, [`MODLET.md`](MODLET.md) (config-validation claim at `MODLET.md:69` matches `RealEarthConfig.Validate()`), [`../SECURITY.md`](../SECURITY.md), or [`BACKUP_RESTORE.md`](BACKUP_RESTORE.md) contradicts the code; the DR doc's checksum/refuse-overwrite claims match `backup_artifacts.sh:61-68,92-119`. Nothing to retract this pass.
...
- **Pipeline-cache poisoning.** A compromised workstation or hostile mirror serves Terrarium PNGs once; they are stored to `data/cache/terrarium` and every future rebuild trusts them (`elevation.py:259-264`), silently baking altered elevation into new packs. Same class as the previous case, offline half of gap 1.
...
- **Client-side enforcement:** none relied on; reveal/debug helpers are config-gated server-side (`MapReveal.cs:38`, `DebugRevealFullMap` default false `RealEarthConfig.cs:59`).
